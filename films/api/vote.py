...
try:
    ...
    if kind == "challenge":
        ...
        if not valid_slug(slug) or target not in ("ai", "human", "rematch"):
            raise ValueError("Invalid challenge vote")
        votes = load_json("votes.json", {})
        votes.setdefault(slug, {"ai": 0, "human": 0, "rematch": 0})
        votes[slug].setdefault("ai", 0)
        votes[slug].setdefault("human", 0)
        votes[slug].setdefault("rematch", 0)
        if not dry_run:
